...
export default function Listing() {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${import.meta.env.VITE_SERVER_URL}api/listing/getListing/${params.listingId}`);
          ...
          <div className='flex flex-col max-w-4xl mx-auto p-3 my-7 gap-4'>
            ...
            </ul>
            {
              currentUser && listing.userRef !== currentUser._id && !contact && (
                <button onClick={()=>setContact(true)} className='bg-slate-700 p-3 rounded-lg uppercase text-white hover:opacity-90 '>Contact Landlord</button>
              )
            }
            {
